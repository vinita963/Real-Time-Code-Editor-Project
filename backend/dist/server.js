...
// Attach WebSocket/Yjs collaboration server
...
/*
Express handles HTTP routes (REST API).
WebSocket server handles collaborative editing.
I mount the Yjs WebSocket server on the same HTTP server
*/
...
app.use((0, cors_1.default)()); // restrict origin in production
app.use(body_parser_1.default.json({ limit: "200kb" }));
...
app.post("/api/complete", async (req, res) => {
    try {
        ...
        const prefix = (body.prefix ?? ""); // text before cursor
        const suffix = (body.suffix ?? ""); // text after cursor, optional
        ...
        // REST call to Gemini (example endpoint — adjust if Google updates it)
        // NOTE: Depending on your Google account/SDK, you may need OAuth2 Bearer token instead of API key.
        // This example uses an API key method in Authorization: Bearer <key> — adjust per your setup.
        const endpoint = "https://generativemodels.googleapis.com/v1/models/gemini-pro:generate";
        const requestPayload = {
            // Model-specific structure; these fields illustrate a common pattern
            prompt: {
                text: prompt
            },
            // control tokens / sampling can go here if needed
...
function extractTextFromGeminiResponse(geminiJson) {
...
function fallbackParseTextToSuggestions(text, maxSuggestions) {
